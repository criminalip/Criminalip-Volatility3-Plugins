...
import os,time, requests,sys,socket,ipaddress,logging,sqlite3
...
class NetworkConnections(interfaces.plugins.PluginInterface, timeliner.TimeLinerInterface):
    ...
    #         cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='ips'")
    #         table_exists = cursor.fetchone()
    
    # # If table doesn't exist, create it
    #         if not table_exists:
    #             cursor.execute('''CREATE TABLE ips (public_ip TEXT,port TEXT,app TEXT, inbound_outbound TEXT, tags TEXT, representative TEXT, ids TEXT, abuse TEXT, verification TEXT)''')
    
    #data table insert
    def table_insert(cursor,ip_data,public_ip,verification):
        ...
        for netw_obj in self.scan(
            ...
            if isinstance(netw_obj, network._TCP_ENDPOINT):
                ...
                if duplicate_rows:
                    ...
                    NetworkConnections.table_insert(cursor, map, foreign,verification)
    ...
    def run(self):
        ...
        conn = sqlite3.connect('C:\\Users\\jisoolee\\volatility3\\db_file.db')
